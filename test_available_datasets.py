...
def test_available_datasets(token):
    ...
    # 測試日期 - 使用最近的日期
    test_date = '2024-01-02'  # 使用 2024-01-02 而不是 2024-01-01
...
def main():
    ...
    # 測試可用的資料集
    successful_datasets, failed_datasets = test_available_datasets(token)
    
    # 儲存結果
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    result_file = f"available_datasets_result_{timestamp}.json"
